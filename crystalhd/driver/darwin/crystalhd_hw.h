...
#ifdef __APPLE__
inline _LIST_STATUS_ operator|(_LIST_STATUS_ a, _LIST_STATUS_ b) {
  return _LIST_STATUS_(int32_t(a) | int32_t(b));
}
inline _LIST_STATUS_& operator|=(_LIST_STATUS_& a, _LIST_STATUS_ b) {
  a = _LIST_STATUS_(int32_t(a) | int32_t(b));
  return a;
}
inline _LIST_STATUS_ operator&(_LIST_STATUS_ a, _LIST_STATUS_ b) {
  return _LIST_STATUS_(int32_t(a) & int32_t(b));
...
inline _LIST_STATUS_ operator~(_LIST_STATUS_ a) {
  return _LIST_STATUS_(~int32_t(a));
}
#endif
...
#else
	semaphore_t fetch_sem; // semaphore between fetch and probe of the next picture information, since both will be in process context
#endif
	bool RxCaptureState;
