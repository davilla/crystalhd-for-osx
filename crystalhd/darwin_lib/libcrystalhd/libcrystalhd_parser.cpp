...
/*				if (timestamp > 0 && DtsGetNaluType(hDevice, pNALU, ulNalSize,&Nalu, true))
				{
					wsprintf(tt, L"DtsAddH264SCode() !! Got NALU_TYPE :%d\n", Nalu.NalUnitType);
					OutputDebugString(tt);		
					if( (Nalu.NalUnitType == NALU_TYPE_SEI) || (Nalu.NalUnitType == NALU_TYPE_PPS) || (Nalu.NalUnitType == NALU_TYPE_SPS))
					{
						wsprintf(tt, L"DtsAddH264SCode() !! Got NALU_TYPE_SEI / NALU_TYPE_PPS /NALU_TYPE_SPS Nalu.Len:%d\n", Nalu.Len);
						OutputDebugString(tt);
						...
						//Copy to Pend Buffer
						memcpy(pPendCurrentPos+BRCM_START_CODE_SIZE, pStart, Nalu.Len);
						//Update Pending Buffer
						pPendCurrentPos = pPendCurrentPos + BRCM_START_CODE_SIZE+Nalu.Len;

						//Update Source Buffer
						pStart = pStart + Ctx->VidParams.StartCodeSz+Nalu.Len;
						lDataRemained = lDataRemained - Ctx->VidParams.StartCodeSz- Nalu.Len;
						
						timestamp = 0;
					}
					else if( (Nalu.NalUnitType == NALU_TYPE_SLICE) | (Nalu.NalUnitType == NALU_TYPE_IDR))
					{
						wsprintf(tt, L"DtsAddH264SCode() !! Got NALU_TYPE_SLICE / NALU_TYPE_IDR Nalu.Len:%d\n", Nalu.Len);
						OutputDebugString(tt);
//						timestamp = 0;
					}
#if 0
					if (
					    ...
					    (Nalu.NalUnitType == NALU_TYPE_SLICE) || 
					    (Nalu.NalUnitType == NALU_TYPE_IDR))
					{
						int i = 0;
						while (1)
						{
							sts = DtsPrepareMdata(Ctx,timestamp,&im, &pSPES, &ulSize);
						...
						if(pSPES && ulSize)
						{
							memcpy(pPendCurrentPos, pSPES, ulSize);
							pPendCurrentPos += ulSize;
							timestamp = 0;
						}				
						sts = DtsInsertMdata(Ctx,im);
						if(sts != BC_STS_SUCCESS){
							wsprintf(tt, L"DtsAddH264SCode() Failed to insert MData\n");
							OutputDebugString(tt);
