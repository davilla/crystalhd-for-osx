...
#define BC_IOC_IOWR(nr,type)	_IOWR(BC_IOC_BASE,nr,type)
// darwin actually uses the size passed into _IOWR to copyin/copyout the type,
// linux does not so we need to fake linux by passing a pointer and do the
// copyin/copyout manually.
#define BC_IOCTL_MB				BC_IOCTL_ARG
#endif
